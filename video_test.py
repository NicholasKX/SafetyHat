...
parser.add_argument("--save_video", type=lambda x: (str(x).lower() == 'true'), default=False,#默认False,
...
video_frame_cnt = int(vid.get(7)) #帧数
...
with tf.Session() as sess:
    ...
    boxes, scores, labels = gpu_nms(pred_boxes, pred_scores, args.num_class,
                                    max_boxes=50, score_thresh=0.65, nms_thresh=0.5)
    ...
    for i in range(video_frame_cnt):
        ...
        cv2.putText(img_ori, '{:.2f}FPS'.format(1/(end_time - start_time)), (40, 40), 0,
